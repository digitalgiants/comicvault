...
def find_matching_comic(db: Session, data: dict) -> Optional[Comic]:
    ...
    for field in ["publisher", "volume", "issue_number", "variant", "cover_letter", "print_run", "printing", "ratio"]:
        val = data.get(field)
        if val is not None:
...
# like the rest.
_COMIC_MATCH_FIELDS = ["publisher", "volume", "issue_number", "variant", "cover_letter", "print_run", "printing", "ratio"]
_COMIC_IDENTITY_FIELDS = ["series", "publisher", "volume", "issue_number", "variant", "cover_letter", "print_run", "printing", "ratio"]
...
def _find_identity_match(db: Session, comic: Comic, updates: dict) -> Optional[Comic]:
    ...
    volume+variant+cover_letter+print_run+printing+ratio matching when
    there's no UPC to go on. Returns None if `updates` doesn't touch any identity field, or
    no match is found."""
