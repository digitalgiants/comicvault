...
- **`/api/tcgs` and `/api/{tcg}/sets` item shapes are still not directly captured**, but the identifier field is very likely `_id` (not `id`) by strong convention, not a guess: apitcg is Mongoose/MongoDB-backed (the confirmed real product response shows `_id`/`__v`/`createdAt`/`updatedAt` throughout, on every nested object), and Mongoose collections key on `_id` universally. Getting this wrong isn't cosmetic - it was live in production: every game fell back to the same empty-string default and silently overwrote a single DB row on every sync, so only one game ("Hololive," in one real run) ever survived. Fixed in `list_games()`/`list_sets()`. `series_external_id`/`release_date` on sets were also fixed to the confirmed real field names (`serie`, `release_date` - snake_case) by analogy with the embedded set object on a real product. `logo_image_url`, `printed_total`, and `total_cards` are still unverified guesses - the embedded per-product objects don't show them either way, so there's nothing to confirm against yet.
